...
import { loadPersistedClasses, savePersistedClasses, loadPersistedStudents, savePersistedStudents, markClassAsDeleted } from '../../utils/rombelPersistence';
...
export function detectImpliedTingkat(namaKelas: string): string | null {
  if (!namaKelas) return null;
  const str = namaKelas.trim().toLowerCase();
  ...
  const romanMatch = str.match(/(?:kelas\s*|kls\s*|^)(xii|xi|x|ix|viii|vii|vi|v|iv|iii|ii|i)(?:\s|[-_]|$)/i);
  if (romanMatch) {
    const rKey = romanMatch[1].toLowerCase();
    if (romanMap[rKey]) return romanMap[rKey];
  }

  // Explicit digit check e.g. "kelas 4", "4a", "4-b", "kelas 12"
  const numMatch = str.match(/(?:kelas\s*|kls\s*|^)(\d{1,2})/i) || str.match(/(\d{1,2})/);
  if (numMatch) {
    const val = parseInt(numMatch[1], 10);
    if (val >= 1 && val <= 12) return val.toString();
  }
...
export default function ManajemenRombel() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // 1. Load Classes
      const loadedClasses = await loadPersistedClasses(scopedClassesKey);
      setClasses(loadedClasses);

      // 2. Load Students
      const scopedStudentsKey = getScopedKey ? getScopedKey('students_list') : `students_list_${activeMadrasahId || 'madrasah_default'}`;
      let loadedStudents: Student[] = await loadPersistedStudents(scopedStudentsKey);
      ...
      console.warn('Fetch Rombel data non-fatal error:', err);
    } finally {
      setLoading(false);
    }
  ...
  const persistClasses = async (newClassesList: RombelClass[]) => {
    ...
    await savePersistedClasses(newClassesList, scopedKey);
  };
  ...
  const persistStudents = async (newStudentsList: Student[]) => {
    ...
    await savePersistedStudents(newStudentsList, scopedKey);
  ...
  const yearClasses = useMemo(() => {
    // Include classes matching selected year OR having no year specified OR set to 'ALL'
    const matching = classes.filter(cls => 
      !cls.tahun_pelajaran || 
      cls.tahun_pelajaran === 'ALL' || 
      cls.tahun_pelajaran === selectedAcademicYear
    );

    const listToUse = matching.length > 0 ? matching : classes;

    const uniqueMap = new Map<string, RombelClass>();
    listToUse.forEach(cls => {
      ...
      } else if (cls.tahun_pelajaran === selectedAcademicYear && existing.tahun_pelajaran !== selectedAcademicYear) {
        uniqueMap.set(key, {
          ...cls,
          tahun_pelajaran: cls.tahun_pelajaran
        });
      }
    });
  ...
  // Mismatched Rombel detection
  const mismatchedRombels = useMemo(() => {
    return yearClasses.filter(c => {
      const implied = detectImpliedTingkat(c.nama_kelas);
      return Boolean(implied && implied !== (c.tingkat || '1'));
    });
  }, [yearClasses]);

  const handleFixRombelTingkat = async (cls: RombelClass, targetTingkat: string) => {
    setSaving(true);
    try {
      const updatedClasses = classes.map(c => c.id === cls.id ? { ...c, tingkat: targetTingkat } : c);
      await persistClasses(updatedClasses);

      const targetYear = cls.tahun_pelajaran || selectedAcademicYear || activeYearFromSettings;
      const isCurrentYear = targetYear === activeYearFromSettings;
      const normName = cls.nama_kelas.trim().toLowerCase();

      const updatedStudents = students.map(s => {
        const { rombel: sRombel, class_id: sClassId } = getStudentRombelForYear(s, targetYear);
        if ((cls.id && sClassId === cls.id) || (sRombel && sRombel.trim().toLowerCase() === normName)) {
          const updated = { ...s };
          if (isCurrentYear) {
            updated.tingkat_rombel = targetTingkat;
          }
          return updated;
        }
        return s;
      ...
      showToast(`Tingkat Rombel "${cls.nama_kelas}" berhasil diperbarui ke Tingkat ${targetTingkat}`, 'success');
    } catch (e) {
      showToast('Gagal memperbarui tingkat rombel', 'error');
    } finally {
      setSaving(false);
    }
  };

  const handleFixAllMismatchedRombels = async () => {
    const mismatched = yearClasses.filter(c => {
      const implied = detectImpliedTingkat(c.nama_kelas);
      return implied && implied !== (c.tingkat || '1');
    });

    if (mismatched.length === 0) {
      showToast('Tidak ada rombel yang salah tingkat.', 'info');
      return;
    }
    ...
    try {
      ...
      mismatched.forEach(m => {
        const implied = detectImpliedTingkat(m.nama_kelas);
        if (implied) fixMap.set(m.id, implied);
      });

      const updatedClasses = classes.map(c => {
        if (fixMap.has(c.id)) {
          return { ...c, tingkat: fixMap.get(c.id)! };
        }
        return c;
      });
      ...
      const isCurrentYear = selectedAcademicYear === activeYearFromSettings;
      const updatedStudents = students.map(s => {
        const { rombel: sRombel, class_id: sClassId } = getStudentRombelForYear(s, selectedAcademicYear);
        const matchedClass = yearClasses.find(c => (sClassId && c.id === sClassId) || (sRombel && c.nama_kelas.trim().toLowerCase() === sRombel.trim().toLowerCase()));
        if (matchedClass && fixMap.has(matchedClass.id)) {
          const newTingkat = fixMap.get(matchedClass.id)!;
          const updated = { ...s };
          if (isCurrentYear) {
            updated.tingkat_rombel = newTingkat;
          }
          return updated;
        }
        return s;
      ...
      await persistStudents(updatedStudents);
      showToast(`Berhasil memperbarui ${mismatched.length} rombel yang salah tingkat!`, 'success');
    } catch (e) {
      showToast('Gagal mengoreksi semua tingkat rombel', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleSaveRombel = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newClassName = rombelForm.nama_kelas!.trim();
      const newTingkat = rombelForm.tingkat || '1';
      const targetYear = rombelForm.tahun_pelajaran || selectedAcademicYear || activeYearFromSettings;

      if (editingRombel) {
        const oldName = editingRombel.nama_kelas?.trim().toLowerCase() || '';
        const oldTingkat = editingRombel.tingkat;
        ...
        // Synchronize students if name or tingkat changed
        if (oldName !== newClassName.toLowerCase() || oldTingkat !== newTingkat) {
          const isCurrentYear = targetYear === activeYearFromSettings;
          const updatedStudents = students.map(s => {
            const { rombel: sRombel, class_id: sClassId } = getStudentRombelForYear(s, targetYear);
            const isMatch = (sClassId && sClassId === editingRombel.id) || (sRombel && sRombel.trim().toLowerCase() === oldName);

            if (isMatch) {
              const updated = setStudentRombelForYear(
                s,
                targetYear,
                newClassName,
                editingRombel.id,
              ...
              if (isCurrentYear) {
                updated.tingkat_rombel = newTingkat;
                updated.rombel = newClassName;
                updated.kelas = newClassName;
              }
              return updated;
            }
            return s;
      ...
      await persistClasses(updatedClasses);
      showToast(editingRombel ? 'Data Rombel & siswa terkait berhasil diperbarui' : 'Rombel baru berhasil dibuat');
  ...
  const executeDeleteRombel = async (clsId: string, clsName: string) => {
    setSaving(true);
    try {
      if (clsId) markClassAsDeleted(clsId);
      if (clsName) markClassAsDeleted(clsName);
      ...
      <div className="space-y-6">
        ...
        {activeTab === 'daftar_rombel' && (
          <div className="space-y-4">
            {/* Mismatched Tingkat Notice Banner if any Rombels have mismatched level */}
            {mismatchedRombels.length > 0 && (
              <div className="bg-amber-500/10 border-2 border-amber-500/30 rounded-2xl p-4 text-amber-900 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-md animate-fadeIn">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-extrabold text-sm text-amber-950 flex items-center gap-2">
                      <span>Peringatan Mismatch Tingkat Rombel ({mismatchedRombels.length} Rombel)</span>
                      <span className="px-2 py-0.5 rounded bg-amber-200 text-amber-900 text-[10px] uppercase font-black">Perlu Perbaikan</span>
                    </h4>
                    <p className="text-xs text-amber-800 mt-0.5 font-medium leading-relaxed">
                      Terdeteksi ketidakcocokan antara nama rombel dan tingkat kelas yang diset.
                      {mismatchedRombels.length === 1 ? (
                        <> Contoh: Rombel <strong>"{mismatchedRombels[0].nama_kelas}"</strong> diset ke <strong>Tingkat {mismatchedRombels[0].tingkat || '1'}</strong> (seharusnya Tingkat {detectImpliedTingkat(mismatchedRombels[0].nama_kelas)}).</>
                      ) : (
                        <> Contoh: Rombel <strong>"{mismatchedRombels[0].nama_kelas}"</strong> diset ke <strong>Tingkat {mismatchedRombels[0].tingkat || '1'}</strong>.</>
                      )}
                    </p>
                  </div>
                </div>
                <div className="flex flex-wrap items-center gap-2 shrink-0 w-full sm:w-auto justify-end">
                  <button
                    type="button"
                    onClick={() => handleOpenEditRombel(mismatchedRombels[0])}
                    className="px-3.5 py-2 bg-amber-100 hover:bg-amber-200 text-amber-900 rounded-xl font-bold text-xs transition-colors flex items-center gap-1.5 border border-amber-300"
                  >
                    <Edit className="w-3.5 h-3.5" /> Edit Rombel
                  </button>
                  <button
                    type="button"
                    disabled={saving}
                    onClick={handleFixAllMismatchedRombels}
                    className="px-4 py-2 bg-amber-600 hover:bg-amber-700 disabled:bg-slate-300 text-white font-black text-xs rounded-xl shadow-md flex items-center gap-1.5 transition-all cursor-pointer"
                  >
                    {saving ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5 text-yellow-300" />}
                    <span>Otomatis Perbaiki Semua ({mismatchedRombels.length})</span>
                  </button>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredRombels.map((rombel) => {
                const impliedTingkat = detectImpliedTingkat(rombel.nama_kelas);
                const isMismatch = Boolean(impliedTingkat && impliedTingkat !== (rombel.tingkat || '1'));

                return (
                  <div
                    key={rombel.id}
                    className={`bg-white rounded-2xl border transition-all p-5 space-y-4 relative flex flex-col justify-between ${
                      isMismatch ? 'border-amber-300 shadow-md ring-1 ring-amber-400/50' : 'border-slate-200/90 shadow-sm hover:shadow-md'
                    }`}
                  >
                    <div className="space-y-3">
                        ...
                        <div>
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className={`text-[10px] font-black px-2 py-0.5 rounded-md uppercase tracking-wider ${
                              isMismatch ? 'bg-amber-100 text-amber-900 border border-amber-300' : 'bg-emerald-100 text-emerald-800'
                            }`}>
                              Tingkat {rombel.tingkat || '1'}
                            </span>
                            {isMismatch && (
                              <span className="text-[10px] font-extrabold px-1.5 py-0.5 rounded bg-rose-100 text-rose-800 border border-rose-200 animate-pulse">
                                ⚠️ Perlu Tingkat {impliedTingkat}
                              </span>
                            )}
                          </div>
                      ...
                      {/* Quick Mismatch Fix Box */}
                      {isMismatch && (
                        <div className="p-2.5 bg-amber-50 border border-amber-200/80 rounded-xl flex items-center justify-between gap-2 text-xs">
                          <div className="text-[11px] font-bold text-amber-800 leading-tight">
                            Nama '{rombel.nama_kelas}' namun diset Tingkat {rombel.tingkat || '1'}.
                          </div>
                          <button
                            type="button"
                            disabled={saving}
                            onClick={() => handleFixRombelTingkat(rombel, impliedTingkat!)}
                            className="px-2.5 py-1 bg-amber-600 hover:bg-amber-700 text-white font-black text-[10px] rounded-lg shadow-sm whitespace-nowrap transition-colors flex items-center gap-1 cursor-pointer shrink-0"
                          >
                            <Sparkles className="w-3 h-3 text-yellow-300" />
                            Fix Ke Tingkat {impliedTingkat}
                          </button>
                        </div>
                      )}
          ...
          <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl overflow-hidden">
              ...
              <form onSubmit={handleSaveRombel} className="p-6 space-y-4 text-xs sm:text-sm">
                ...
                {/* Smart Level Mismatch Notice in Modal */}
                {(() => {
                  const implied = detectImpliedTingkat(rombelForm.nama_kelas || '');
                  if (implied && implied !== (rombelForm.tingkat || '1')) {
                    return (
                      <div className="p-3 bg-amber-50 border border-amber-300/80 rounded-xl space-y-2">
                        <div className="flex items-center gap-2 text-amber-900 font-bold text-xs">
                          <AlertCircle className="w-4 h-4 text-amber-600 shrink-0" />
                          <span>Deteksi Mismatch Tingkat Rombel</span>
                        </div>
                        <p className="text-[11px] text-amber-800 leading-normal font-medium">
                          Nama Rombel <strong>"{rombelForm.nama_kelas}"</strong> terdeteksi sebagai <strong>Tingkat {implied}</strong>, tetapi Tingkat terpilih adalah <strong>Tingkat {rombelForm.tingkat || '1'}</strong>.
                        </p>
                        <button
                          type="button"
                          onClick={() => setRombelForm({ ...rombelForm, tingkat: implied })}
                          className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white text-[11px] font-extrabold rounded-lg shadow-sm transition-colors flex items-center gap-1 cursor-pointer"
                        >
                          <Sparkles className="w-3.5 h-3.5 text-yellow-300" />
                          Sesuaikan Tingkat ke {implied}
                        </button>
                      </div>
                    );
                  }
