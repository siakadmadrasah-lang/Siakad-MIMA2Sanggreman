...
import { loadPersistedClasses, loadPersistedStudents } from '@/utils/rombelPersistence';
...
const isGraduatedStudent = (student: any) => {
  ...
  const rombel = (student.rombel || student.kelas || student.tingkat_rombel || '').toLowerCase().trim();
  if (status === 'graduated' || status === 'lulus' || status === 'alumni' || status === 'tamat') return true;
  if (rombel.includes('alumni') || rombel.includes('lulus')) return true;
  if (Array.isArray(student.riwayat_rombel)) {
    return student.riwayat_rombel.some((h: any) => {
      const hStatus = (h?.status || '').toLowerCase().trim();
      const hRombel = (h?.rombel || '').toLowerCase().trim();
      return hStatus === 'graduated' || hStatus === 'lulus' || hStatus === 'alumni' || hRombel.includes('alumni') || hRombel.includes('lulus');
    });
  }
  return false;
};
...
const DataTamatan = () => {
  ...
  const mergeTamatanData = useCallback((savedList: TamatanItem[], studentsList: any[], classesList: any[]) => {
    ...
    alumniData.forEach((student: any) => {
      ...
      if (!yr && Array.isArray(student.riwayat_rombel)) {
        const gradHist = student.riwayat_rombel.find((h: any) => {
          const st = (h?.status || '').toLowerCase().trim();
          return st === 'graduated' || st === 'lulus' || st === 'alumni';
        });
        if (gradHist?.tahun_pelajaran) {
          yr = gradHist.tahun_pelajaran.trim();
        }
      }
      if (!yr) {
    ...
    Object.keys(alumniMap).forEach((yr) => {
      ...
      if (existingIdx >= 0) {
        // If DB has alumni recorded for this year, sync the count
        mergedList[existingIdx] = {
          ...
          keterangan: mergedList[existingIdx].keterangan && !mergedList[existingIdx].keterangan.includes('Proyeksi')
            ? mergedList[existingIdx].keterangan
            : 'Otomatis dari Database Alumni',
        };
      } else {
        mergedList.unshift({
  ...
  const fetchTamatan = useCallback(async () => {
    ...
    try {
      ...
      let studentsList = await loadPersistedStudents(studentsKey);
      if (!studentsList || studentsList.length === 0) {
        studentsList = localStudents;
      }

      let classesList = await loadPersistedClasses(classesKey);
