...
import { 
  ...
  Cloud, BookOpen, MessageCircle, Share2, Camera, X, ImageIcon, Printer
} from 'lucide-react';
...
const LinksAdmin = () => {
  ...
  return (
    <AdminLayout title="Manajemen Pusat Tautan">
      {/* Printable Area for Links Directory Document */}
      <div className="hidden print:block font-serif text-black p-6 space-y-6">
        <KopSurat />

        <div className="text-center my-4">
          <h2 className="text-lg font-bold underline uppercase">DIREKTORI TAUTAN & LAYANAN DIGITAL RESMI MADRASAH</h2>
        ...
        <div className="space-y-4 text-xs">
          {groups.map((group, gIdx) => (
            <div key={group.id} className="border border-black p-3 rounded">
              <h3 className="font-bold text-sm border-b border-black pb-1 mb-2 bg-gray-100 p-1">
              ...
              <table className="w-full border-collapse border border-black">
                <thead>
                  <tr className="bg-gray-50">
                    ...
                    <th className="border border-black p-1 text-left w-1/3">Nama Layanan / Tautan</th>
                    <th className="border border-black p-1 text-left">Deskripsi</th>
                    <th className="border border-black p-1 text-left w-1/3">Alamat URL</th>
                  </tr>
                </thead>
                <tbody>
                  {group.links.map((link, lIdx) => (
                    <tr key={link.id}>
                      <td className="border border-black p-1 text-center">{lIdx + 1}</td>
                      <td className="border border-black p-1 font-semibold">{link.title}</td>
                      <td className="border border-black p-1">{link.desc || '-'}</td>
                      <td className="border border-black p-1 font-mono text-[10px] break-all">{link.url}</td>
                    </tr>
                  ))}
                  {group.links.length === 0 && (
                    <tr>
                      <td colSpan={4} className="border border-black p-2 text-center italic text-gray-500">
                        Belum ada tautan dalam kategori ini.
                      </td>
                    </tr>
                  )}
                </tbody>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8 print:hidden">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => window.print()} className="border-emerald-200 text-emerald-700 hover:bg-emerald-50">
            <Printer className="w-4 h-4 mr-2" /> Cetak Direktori
          </Button>
