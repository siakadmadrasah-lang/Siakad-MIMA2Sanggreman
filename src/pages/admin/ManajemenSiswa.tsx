...
export default function ManajemenSiswa() {
  ...
  const [studentMediaModalOpen, setStudentMediaModalOpen] = useState(false);
  ...
  const handleStudentPhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const url = await uploadImageToStorage(file, 'siswa');
      if (url) {
        setStudentForm(prev => ({ ...prev, foto_url: url, photo_url: url }));
        showNotice("Foto siswa berhasil diunggah!", "success");
      } else {
        showNotice("Gagal mengunggah foto siswa", "error");
      }
    } catch (err) {
      console.error(err);
      showNotice("Gagal memproses foto siswa", "error");
    } finally {
  ...
  const handleOpenEditModal = (student: Student) => {
    ...
    setStudentForm({
      ...
      foto_url: student.foto_url || student.photo_url || '',
      photo_url: student.foto_url || student.photo_url || '',
    });
    setShowAddModal(true);
  };
  ...
  return (
    <AdminLayout title="Modul Manajemen Siswa Mandiri">
                    ...
                    filteredStudents.map((student) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="p-4">
                            <div className="flex items-center gap-3">
                              {(student.foto_url || student.photo_url) ? (
                                <img
                                  src={formatImageUrl(student.foto_url || student.photo_url || '')}
                                  alt={student.name}
                                  className="w-10 h-10 rounded-full object-cover border border-emerald-300 shadow-sm shrink-0"
                                />
                              ) : (
                                <div className="w-10 h-10 rounded-full bg-emerald-100 text-emerald-800 flex items-center justify-center font-extrabold text-sm shrink-0 border border-emerald-200">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden animate-scaleIn my-8">
            ...
            <form onSubmit={handleSaveStudent} className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
              ...
              {/* Section 3: Foto Siswa */}
              <div>
                <h4 className="font-bold text-sm text-emerald-900 border-b pb-2 mb-4 flex items-center gap-2">
                  <ImageIcon className="w-4 h-4 text-emerald-600" /> Foto Siswa / Pas Foto
                </h4>
                <div className="flex items-center gap-4 bg-emerald-50/50 p-4 rounded-xl border border-emerald-100">
                  {(studentForm.foto_url || studentForm.photo_url) ? (
                    <img
                      src={formatImageUrl(studentForm.foto_url || studentForm.photo_url || '')}
                      alt="Preview Foto"
                      className="w-16 h-20 rounded-xl object-cover border-2 border-emerald-300 shadow-sm shrink-0"
                    />
                  ) : (
                    <div className="w-16 h-20 rounded-xl bg-gray-100 flex items-center justify-center text-gray-400 border border-dashed border-gray-300 shrink-0">
                      <ImageIcon className="w-8 h-8" />
                    </div>
                  )}
                  <div className="flex flex-wrap items-center gap-2">
                    <label className="cursor-pointer">
                      <span className="inline-flex items-center px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs transition-colors shadow-sm">
                        <Upload className="w-3.5 h-3.5 mr-1.5" /> {uploadingStudentPhoto ? 'Mengunggah...' : 'Upload Foto'}
                      </span>
                      <input
                        type="file"
                        accept="image/*"
                    ...
                    <button
                      type="button"
                      onClick={() => setStudentMediaModalOpen(true)}
                      className="px-3 py-2 bg-white hover:bg-gray-50 text-gray-700 font-bold rounded-xl text-xs border border-gray-200 transition-colors shadow-sm"
                    >
                      Pilih dari Galeri
                    </button>
                    {(studentForm.foto_url || studentForm.photo_url) && (
                      <button
                        type="button"
                        onClick={() => setStudentForm(prev => ({ ...prev, foto_url: '', photo_url: '' }))}
                        className="px-3 py-2 bg-rose-50 hover:bg-rose-100 text-rose-600 font-bold rounded-xl text-xs transition-colors border border-rose-200"
                      >
                        Hapus Foto
                      </button>
                    )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-scaleIn">
            <div className="bg-gradient-to-r from-emerald-800 to-teal-800 text-white p-6 relative">
              ...
              <div className="flex items-center gap-4">
                {(viewingStudent.foto_url || viewingStudent.photo_url) ? (
                  <img
                    src={formatImageUrl(viewingStudent.foto_url || viewingStudent.photo_url || '')}
                    alt={viewingStudent.name}
                    className="w-16 h-20 rounded-2xl object-cover border-2 border-white/40 shadow-md shrink-0"
                  />
                ) : (
                  <div className="w-16 h-16 rounded-2xl bg-white/20 border-2 border-white/40 flex items-center justify-center font-bold text-2xl text-yellow-300 shrink-0">
      ...
      {/* MEDIA LIBRARY MODAL FOR STUDENT PHOTO */}
      {studentMediaModalOpen && (
        <MediaLibraryModal
          isOpen={studentMediaModalOpen}
          onClose={() => setStudentMediaModalOpen(false)}
          onSelectMedia={(url) => {
            setStudentForm(prev => ({ ...prev, foto_url: url, photo_url: url }));
            setStudentMediaModalOpen(false);
            showNotice("Foto siswa berhasil dipilih dari Galeri!", "success");
          }}
        />
      )}
    </AdminLayout>
