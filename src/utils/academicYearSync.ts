...
import { loadPersistedClasses, savePersistedClasses } from './rombelPersistence';
...
export async function syncDataToAcademicYear(
  ...
  try {
    // 1. Fetch current student list from Database / localStorage
    ...
    const currentClasses = await loadPersistedClasses(scopedClassesKey);
    ...
    if (updatedClasses.length > 0) {
      await savePersistedClasses(updatedClasses, scopedClassesKey);
    }
